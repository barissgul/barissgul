<?
	...
	$row 				= $cSubData->getVirman($_REQUEST);
?>
<!DOCTYPE html>
...
<body class="mod-bg-1">
        ...
        <section class="content">
	    	...
	    	<div class="panel">
	            <div class="panel-hdr bg-primary-gradient">
	                <h2> <i class="fal fa-lira-sign mr-3"></i> <?=dil("Virman Dekontu")?> </h2>
	            ...
	            <div class="panel-container show">
	                <div class="panel-content">
	                	<ul class="nav nav-tabs justify-content-center" role="tablist">
		                   	<li class="nav-item"><a class="nav-link fs-md py-3 px-4 <?=($_REQUEST['tab']=='tab_kasa')?'active':''?>" href="#tab_kasa" data-toggle="tab"> <?=dil("Kasa Virman")?> </a></li>
		                   	<li class="nav-item"><a class="nav-link fs-md py-3 px-4 <?=($_REQUEST['tab']=='tab_cari')?'active':''?>" href="#tab_cari" data-toggle="tab"> <?=dil("Cari Virman")?> </a></li>
		                </ul>
		                <div class="tab-content p-3 shadow">
						    <div class="tab-pane <?=($_REQUEST['tab']=='tab_kasa')?'active':''?>" id="tab_kasa">	
								<form name="formKasaVirman" id="formKasaVirman" class="" enctype="multipart/form-data" method="POST">
									<input type="hidden" name="islem" value="finans_kasa_virman_ekle">
									...
									<div class="row">
											...
											<div class="row">
												<div class="col-sm-4 col-xs-4 mb-2"> 					            
												    <div class="form-group">
													  	<label class="form-label"><?=dil("Borç Kasa")?></label>
													  	<select name="borc_kasa_id" id="borc_kasa_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
													      	<?=$cCombo->OdemeKanalDetaylar()->setSecilen($row->BORC_KASA_ID)->setTumu()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
												<div class="col-sm-4 col-xs-4 mb-2"> 					            
												    <div class="form-group">
													  	<label class="form-label"><?=dil("Alacak Kasa")?></label>
													  	<select name="alacak_kasa_id" id="alacak_kasa_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
													      	<?=$cCombo->OdemeKanalDetaylar()->setSecilen($row->ALACAK_KASA_ID)->setTumu()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
												<div class="w-100"></div>
										...
										<div class="col-md-12 col-sm-12 col-xs-12 text-center mt-3">
											<div class="form-group">
										    	...
										    	<button type="button" class="btn btn-success btn-ozel" onclick="fncKasaKaydet()"><?=dil("Kaydet")?></button>
										    </div>
										</div>
										<?}?>
									</div>										
							...
							<div class="tab-pane <?=($_REQUEST['tab']=='tab_cari')?'active':''?>" id="tab_cari">
								<form name="formCariVirman" id="formCariVirman" class="" enctype="multipart/form-data" method="POST">
									<input type="hidden" name="islem" value="finans_cari_virman_ekle">
									<input type="hidden" name="id" value="<?=$row->ID?>">
									<input type="hidden" name="kod" value="<?=$row->KOD?>">
									<input type="hidden" name="form_key" value="<?=fncFormKey()?>">
									...
									<div class="row">
			                        	<div class="col-sm-12">
											<div class="row">
												<div class="col-sm-4 col-xs-4 mb-2"> 					            
												    <div class="form-group">
													  	<label class="form-label"><?=dil("Borç Cari")?></label>
													  	<select name="borc_cari_id" id="borc_cari_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
													      	<?=$cCombo->Cariler()->setSecilen($row->BORC_ID)->setTumu()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
												<div class="col-sm-4 col-xs-4 mb-2"> 					            
												    <div class="form-group">
													  	<label class="form-label"><?=dil("Alacak Cari")?></label>
													  	<select name="alacak_cari_id" id="alacak_cari_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
													      	<?=$cCombo->Cariler()->setSecilen($row->ALACAK_ID)->setTumu()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
												<div class="w-100"></div>
										...
										<div class="col-md-12 col-sm-12 col-xs-12 text-center mt-3">
											<div class="form-group">
										      	<label class="form-label"></label>
										    	<button type="button" class="btn btn-success btn-ozel" onclick="fncCariKaydet()"><?=dil("Kaydet")?></button>
										    </div>
										</div>
										<?}?>
									</div>										
    ...
    <script>
		...
		function fncKasaKaydet(obj){
			$(obj).attr("disabled", "disabled");
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: $('#formKasaVirman').serialize(),
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){
		...
		function fncCariKaydet(obj){
			$(obj).attr("disabled", "disabled");
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: $('#formCariVirman').serialize(),
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){

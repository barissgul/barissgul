<?
	...
	$excel->sutunEkle("Ödenen Cari","CARI","");
	...
	$Table = $cTable->setSayfa("sayfaTediyeler")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();
	$rows = $Table['rows'];
								...
								<div class="row">
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
									      	...
									      	<input type="text" class="form-control" placeholder="" name="talep_no" id="talep_no" value="<?=$_REQUEST['talep_no']?>" maxlength="12">
									    </div>
									</div>
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
		    	...
		    	<div class="col-md-12">
		    		<div class="panel">
	                    <div class="panel-hdr bg-primary-300 text-white">
	                        <h2> <i class="fal fa-lira-sign mr-3"></i> <?=dil("Tediyeler")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
	                        <div class="panel-toolbar">
	                            <a href="javascript:void(0)" onclick="fncPopup('/finans/tediye.do?route=finans/tediyeler','TEDIYE_EKLE',1000,800)" class="btn btn-outline-secondary btn-icon waves-effect waves-themed border-white text-white mr-2" title="Tediye Ekle"> <i class="far fa-plus"></i> </a>
							  	...
							  	<table class="table table-sm table-condensed table-hover">
							        ...
							        <tbody>
								        ...
								        foreach($rows as $key=>$row) {
								        	$row_toplam->KDVSIZ_TUTAR += $row->KDVSIZ_TUTAR;
								        	$row_toplam->KDV_TUTAR += $row->KDV_TUTAR;
								        	$row_toplam->TUTAR += $row->TUTAR;
								        	?>
									        <tr>
									          	<td class="thead-themed" align="center"><?=($Table['sayfaIlk']+$key+1)?></td>
									          	<td><?=$row->ID?></td>
									          	...
									          	<td align="center" class="p-0" nowrap>
									          		<a href="<?=fncOdemePopupLink($row)?>" class="btn btn-outline-primary btn-icon"> <i class="fal fa-eye"></i> </a>
									          		<a href="javascript:void(0)" onclick="fncPopup('/finans/tediye.do?route=finans/tediyeler&id=<?=$row->ID?>&kod=<?=$row->KOD?>','TEDIYE',1000,800)" class="btn btn-outline-primary btn-icon" title="Ödeme"> <i class="far fa-calculator"></i> </a>
									          	</td>	
									        </tr>
								        <?}?>
								        <tr class="thead-themed">
								          	...
								          	<td align="right"> <?=FormatSayi::db2tr($row_toplam->KDVSIZ_TUTAR)?> </td>
								          	<td align="right"> <?=FormatSayi::db2tr($row_toplam->KDV_TUTAR)?> </td>

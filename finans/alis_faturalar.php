<?
	...
	$Table = $cTable->setSayfa("sayfaAlisFaturalar")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();
	$rows = $Table['rows'];
		    	...
		    	<div class="col-md-12">
		    		<div class="panel">
	                    <div class="panel-hdr bg-primary-300 text-white">
	                        <h2> <i class="fal fa-list mr-3"></i>  <?=dil("Alış Faturaları")?>&nbsp;<span style="font-size: 10px;">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
	                        <div class="panel-toolbar">
	                            <a href="/finans/alis_fatura.do?route=finans/alis_faturalar" class="btn btn-icon btn-outline-secondary text-white border-white mr-1" title="Ekle"> <i class="far fa-plus"></i> </a>
							        ...
							        <tbody>
								        ...
								        foreach($rows as $key=>$row) {
									        ...
									        <tr>
									          	...
									          	<td align="center"> <a href="/finans/alis_fatura.do?route=finans/alis_faturalar&id=<?=$row->ID?>&kod=<?=$row->KOD?>" class="btn <?=($row->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-xs fs-lg"> <?=$row->ID?></a> </td>
									          	...
									          	<td><?=$row->FATURA_NO?></td>
									          	<td nowrap><?=$row->FINANS_KALEMI?></td>
									          	<td><?=$row->PLAKA?></td>
									          	<td><?=$row->CARI_KODU?></td>

<?
	...
	$_REQUEST['tarih'] = $_REQUEST['tarih'] ? $_REQUEST['tarih'] : date("d-m-Y , d-m-Y");
	
	$rows_tahsilat 			= $cSubData->getGunlukTahsilat($_REQUEST);
	$rows_toplam_tahsilat 	= $cSubData->getGunlukTahsilatToplam($_REQUEST);
	
	$rows_tediye			= $cSubData->getGunlukTediye($_REQUEST);
	$rows_toplam_tediye		= $cSubData->getGunlukTediyeToplam($_REQUEST);
	...
	$rows_kasa			= $cSubData->getKasalarGunluk($_REQUEST);
?>
<!DOCTYPE html>
    	...
    	<section class="content">		    
	    	<div class="row">
									...
									<div class="row">
										<div class="col-md-8 mb-2">        
									        <div class="form-group">
									        	<label class="form-label" for="tarih_var"><?=dil("Tarih")?></label>
										...
										<div class="col-md-2">
											<div class="form-group">
											  	<label> &nbsp; </label><br>
										  		<button type="button" class="btn btn-primary waves-effect waves-themed" onclick="fncMailGonder(this)"><?=dil("Mail Gönder")?></button>
										  	</div>
										</div>
									</div>
								</form>
							  	...
							  	<table class="table table-sm table-condensed table-hover">
							  		<thead class="thead-themed">
								  		<tr>
											...
											<td align="right"> <b><?=dil("Limit")?></b> </td>
									...
									<tbody>
								  		...
								  		foreach($rows_kasa as $key => $row_kasa){
											...
											<tr>
												...
												<td align="right"> <?=FormatSayi::sayi($row_kasa->LIMIT,2)?> </td>
		    	...
		    	<div class="col-lg-6 col-md-6">
		    		<div class="panel">
	                    <div class="panel-hdr bg-primary-300 text-white">
	                        <h2> <?=dil("TAHSİLATLAR - Gelirler")?> </h2>
	                        <div class="panel-toolbar">
	                        	<a href="javascript:void(0)" onclick="fncPopup('/finans/tahsilat.do?route=finans/tahsilat','TAHSILAT_EKLE',1000,800)" class="btn btn-icon btn-light mr-1" title="Gelir Ekle"> <i class="far fa-plus"></i> </a>
	                            <button class="btn btn-panel waves-effect waves-themed" data-action="panel-collapse" data-toggle="tooltip" data-offset="0,10" data-original-title="Küçült"></button>
				       			<button class="btn btn-panel waves-effect waves-themed" data-action="panel-fullscreen" data-toggle="tooltip" data-offset="0,10" data-original-title="Tam Ekran"></button>
				        		<button class="btn btn-panel waves-effect waves-themed" data-action="panel-close" data-toggle="tooltip" data-offset="0,10" data-original-title="Kapat"></button>
	                        </div>
                                ...
                                <div class="panel-tag">
							  	...
							  	<table class="table table-sm table-condensed table-hover fs-xs">
									...
									<tbody>
										...
										<tr>
											<td align="center"> <a href="javascript:fncPopup('/finans/tahsilat.do?route=finans/tahsilat&id=<?=$row_tahsilat->ID?>&kod=<?=$row_tahsilat->KOD?>','TAHSILAT',1000,800 )" class="btn <?=($row_tahsilat->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-sm"> <?=$row_tahsilat->ID?></a> </td>
			    ...
			    <div class="col-lg-6 col-md-6">
			    	<div class="panel">
	                    <div class="panel-hdr bg-primary-300 text-white">
	                        <h2> <?=dil("TEDİYELER - Giderler")?> </h2>
	                        <div class="panel-toolbar">
	                        	<a href="javascript:void(0)" onclick="fncPopup('/finans/tediye.do?route=finans/tediyeler','TEDIYE_EKLE',1000,800)" class="btn btn-icon btn-light mr-1" title="Gider Ekle"> <i class="far fa-plus"></i> </a>
	                           	<button class="btn btn-panel waves-effect waves-themed" data-action="panel-collapse" data-toggle="tooltip" data-offset="0,10" data-original-title="Küçült"></button>
						        <button class="btn btn-panel waves-effect waves-themed" data-action="panel-fullscreen" data-toggle="tooltip" data-offset="0,10" data-original-title="Tam Ekran"></button>
						        <button class="btn btn-panel waves-effect waves-themed" data-action="panel-close" data-toggle="tooltip" data-offset="0,10" data-original-title="Kapat"></button>
	                        </div>
						        ...
						        <div class="panel-tag">
							  	...
							  	<table class="table table-sm table-condensed table-hover fs-xs">
									...
									<tbody>
										...
										<tr>
											<td align="center"> <a href="javascript:fncPopup('/finans/tediye.do?route=finans/tediye&id=<?=$row_tediye->ID?>&kod=<?=$row_tediye->KOD?>','TEDIYE',1000,800 )" class="btn <?=($row_tediye->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-sm"> <?=$row_tediye->ID?></a> </td>
    ...
    <script>
		...
		function fncMailGonder(obj){
			...
			$.ajax({
				...
				type: "POST",
				data: { "islem" : "gunluk_kasa_mail_gonder", 'tarih' : $("#tarih").val() },
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){

<?
	...
	$Table = $cTable->setSayfa("sayfaServisler")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Servis Listesi")?></li>
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2> <?=dil("Servis Listesi")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
						  		...
						  		<table class="table table-sm table-condensed table-hover">
							  		<thead class="thead-themed fw-500">
								    	<tr>
								          	...
								          	<td align="center">#</td>
								          	<td><?=dil("Ünvan")?></td>
								          	<td><?=dil("Servis Türü")?></td>
								          	<td><?=dil("Servis Zinciri")?></td>
								          	<td><?=dil("Vergi Kimlik No")?></td>
								          	<td><?=dil("İl")?></td>
								          	<td><?=dil("İlçe")?></td>
								          	<td align="center"><?=dil("Vale")?></td>
								          	<td align="center"><?=dil("Onarım")?></td>
								          	<td align="center"><?=dil("Bakım")?></td>
								          	<td align="center"><?=dil("Cam")?></td>
								          	<td align="center"><?=dil("Lastik")?></td>
								          	<td align="center"><?=dil("L.Otel")?></td>
								          	<td align="center"><?=dil("Akü")?></td>
							        ...
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td><?=$row->UNVAN?></td>
									          	<td><?=$row->SERVIS_TURU?></td>
									          	<td><?=$row->SERVIS_ZINCIR?></td>
									          	<td><?=$row->TCK?></td>
									          	<td><?=$row->IL?></td>
									          	<td><?=$row->ILCE?></td>
									          	<td align="center"><?=fncVarYok($row->VALE_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->ONARIM_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->BAKIM_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->CAM_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->LASTIK_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->LASTIK_OTEL_HIZMETI)?></td>
									          	<td align="center"><?=fncVarYok($row->AKU_HIZMETI)?></td>
									          	<td align="center"><?=$row->DURUM?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->TARIH)?></td>
									          	<td align="right" class="p-0"> 
									          		<a href="/tanimlama/servis.do?route=<?=$_REQUEST['route']?>&id=<?=$row->ID?>&kod=<?=$row->KOD?>" class="btn btn-outline-primary btn-icon" title="Kullanıcı Düzenle"> <i class="far fa-edit"></i> </a> 
									          	</td>
									        </tr>
								        <?}?>
							        </tbody>

<?
	...
	$Table = $cTable->setSayfa("sayfaZincirServisler")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
								        ...
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td align="right" class="p-0"> 
									          		<a href="/tanimlama/servis_zincir.do?route=<?=$_REQUEST['route']?>&id=<?=$row->ID?>&kod=<?=$row->KOD?>" class="btn btn-outline-primary btn-icon" title="Kullanıcı Düzenle"> <i class="far fa-edit"></i> </a> 
									          	</td>
									        </tr>
								        <?}?>
							        </tbody>

<?
	...
	$Table = $cTable->setSayfa("sayfaUrunler")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();
	$rows = $Table['rows'];
...
<body class="mod-bg-1">
								...
								<div class="row">
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
									      	<label class="form-label"> <?=dil("Ürün Kodu")?> </label>
									      	<input type="text" class="form-control" placeholder="" name="kodu" id="kodu" value="<?=$_REQUEST['kodu']?>" maxlength="30">
									    </div>
									</div>
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
									      	<label class="form-label"> <?=dil("Ürün Adı")?> </label>
									      	<input type="text" class="form-control" placeholder="" name="urun" id="urun" value="<?=$_REQUEST['urun']?>" maxlength="50">
									    </div>
									</div>
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
	    	...
	    	<div class="row">
		    	<div class="col-xl-12">
                    ...
                    <div class="panel-hdr bg-primary-300 text-white">
                        <h2> <i class="fal fa-cog fa-2x mr-3"></i> <b> <?=dil("Ürünler")?> </b> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
                        <div class="panel-toolbar">
                        	<a href="javascript:fncPopup('/urun/popup_urun.do?route=urun/popup_urun','POPUP_URUN',1100,850);" class="btn btn-outline-secondary text-white border-white btn-icon mr-1"> <i class="far fa-plus"></i> </a>
						        ...
						        <tbody>
							        ...
							        foreach($rows as $key=>$row) {
								        ...
								        <tr>
								          	...
								          	<td> <a href="javascript:fncPopup('/urun/urun_detay.do?route=urun/urun_detay&parca_kodu=<?=$row->KODU?>&servis_id=<?=$row->SERVIS_ID?>&filtre=1','STOK_DETAY',1200,800);" title="Ektre"> <?=$row->KODU?> </a> </td>
								          	...
								          	<td class="text-right p-0 text-center" nowrap> 
								          		<a href="javascript:fncPopup('/urun/popup_urun.do?route=urun/popup_urun&id=<?=$row->ID?>&kod=<?=$row->KOD?>','POPUP_URUN',1200,850);" class="btn btn-md btn-outline-primary btn-icon"> <i class="far fa-pencil"></i> </a> 
								          		<a href="javascript:fncPopup('/urun/popup_urun_cikti.do?route=urun/popup_urun_cikti&id=<?=$row->ID?>&kod=<?=$row->KOD?>','POPUP_YAZDIR',1200,800);" class="btn btn-md btn-outline-primary btn-icon" title="Yazdır"> <i class="fal fa-print"></i></a>
			                					<a href="javascript:fncPopup('/urun/popup_urun_etiket.do?route=urun/popup_urun_etiket&id=<?=$row->ID?>&kod=<?=$row->KOD?>','POPUP_YAZDIR_ETIKET',1200,800);" class="btn btn-md btn-outline-primary btn-icon" title="Etiket"> <i class="fal fa-barcode"></i></a>
								          	</td>
								        </tr>
							        <?}?>
    ...
    <script>
		...
		function fncFiltrele(){
			if($("#katalog").val() == "" && $("#oem_kodu").val() == "" && $("#urun").val() == "" && $("#tedarikci_id").val() <= 0){
		...
		$("#marka, #kodu, #oem_kodu, #urun, #barkod").on('keyup', function (e) {
		    if (e.keyCode == 13) {
		        fncFiltrele();
		    }
		});
		...
		function fncStokDuzenle(obj){
			$.ajax({
				...
				type: "POST",
				data: { "islem" : "urun_bilgisi", 'id' : $(obj).data("id") },
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){

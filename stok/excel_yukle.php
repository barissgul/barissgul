...
<head>
    <meta charset="utf-8">
    <title> <?=$row_site->TITLE?> <?=dil("Excel Yükle")?></title>
...
<body class="<?=$cBootstrap->getBody()?>">
                        	 ...
                        	 <table class="table table-sm table-condensed table-hover">
							    ...
							    <tbody>
							        ...
							        <tr>
							          	...
							          	<td><?=FormatYazi::kisalt($row_excel->TEDARIKCI,10)?><br><?=$row_excel->PARCA_TIPI?></td>
							          	<td><?=FormatSayi::sayi($row_excel->LISTE_KAR_ORANI)?></td>
							          	...
							          	<td style="padding: 0px;" align="center">								          		
							          		...
							          		<?}?>
							          		<a href="javascript:fncPopup('/stok/excel_in.do?id=<?=$row_excel->ID?>&kod=<?=$row_excel->KOD?>','EXCEL_IN',1250,800)" class="btn btn-success btn-sm" title="Uygula"><i class="fas fa-play"></i></a> 
							          		<!--
							          		<a href="javascript:fncPopup('http://tedarikekspar.boryaz.com/yedek_parca/excel_in.do?id=<?=$row_excel->ID?>&kod=<?=$row_excel->KOD?>','EXCEL_IN',1250,800)" class="btn btn-success btn-sm" title="Uygula"><i class="fas fa-play"></i></a> 
							          		-->
							          	</td>
							        </tr>
							        <?}?>
	                        	...
	                        	<div class="row">
									<div class="col-lg-6 col-md-6 mt-2">
										<div class="form-group">
										  	...
										  	<select name="parca_tipi_id" id="parca_tipi_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;" onchange="fncParcaTipiSec(this)">
										      	<?=$cCombo->ParcaTipi()->setSecilen()->setSeciniz()->getSelect("ID","AD")?>
										    </select>
										</div>
									</div>
									<div class="col-lg-6 col-md-6 mt-2">
									    <div class="form-group">
									      	<label class="form-label"> <?=dil("Liste Kar Oranı")?> </label>
									      	<div class="input-group">
									      		<input type="text" class="form-control" placeholder="" name="liste_kar_orani" id="liste_kar_orani" data-inputmask="'alias': 'decimal', 'groupSeparator': '.', 'autoGroup': true, 'radixPoint': ',', 'digits': 2" data-mask style="text-align: right;" value="<?=FormatSayi::sayi($row->LISTE_KAR_ORANI,2)?>" maxlength="12">
									...
									<div class="col-md-12 mt-2">
										<div class="panel-tag">
								            ...
								            <p>Örnek Excel Şablon dosyası: <a href="/stok/stok_liste.xlsx"> <i class="fas fa-download fa-2x"></i> </a></p>
								        </div>
									</div>
								</div>
							</div>
    ...
    <script>
		...
		$("#excel").fileinput({
			theme: 'explorer-fas',
	    	language: 'tr',
	        uploadUrl: '/class/db_kayit.do?islem=stok_excel_yukle',
	        ...
	        uploadExtraData: function() {
			    return {
			    	...
			    	parca_tipi_id: $('#parca_tipi_id').val(),
			    	liste_kar_orani: $('#liste_kar_orani').val(),
		...
		$('#excel').on('filebatchuploadcomplete', function(event, data, previewId, index) {
		   	location.href = "/stok/excel_yukle.do?route=stok/excel_yukle";
		});
		
		function fncParcaTipiSec(obj){
			if($(obj).val() > 0){

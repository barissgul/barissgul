<?
	...
	$excel->sutunEkle("Kimlik No","TCK","");
	$excel->sutunEkle("Personel","CARI","");
	$excel->sutunEkle("Departman","DEPARTMAN","");
	$excel->sutunEkle("Görev","GOREV","");
	$excel->sutunEkle("Maaş Dönem","MAAS_DONEM","");
	$excel->sutunEkle("Adres İl","ADRES_IL","");
	$excel->sutunEkle("Adres İlçe","ADRES_ILCE","");
	...
	$Table = $cTable->setSayfa("sayfaPersoneller")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Personel Listesi")?></li>
                        ...
                        <div class="panel-content">
							<form name="form" id="form" class="app-forms" enctype="multipart/form-data" method="GET">
								...
								<div class="row">
									...
									<div class="col-lg-2 col-md-3 col-sm-6 col-xs-6 mb-2">        
									    <div class="form-group">
									      	<label class="form-label"> <?=dil("Personel")?> </label>
									...
									<div class="col-lg-2 col-md-3 mb-2">						            
									    <div class="form-group">
										  	...
										  	<select name="durum" id="durum" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
										      	<?=$cCombo->Durumlar()->setSecilen(isset($_REQUEST['durum'])?$_REQUEST['durum']:1)->setTumu()->getSelect("ID","AD")?>
										    </select>
										</div>
									</div>
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2> <i class="fal fa-user mr-3"></i> <?=dil("Personel Listesi")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
                        <div class="panel-toolbar">
                        	<!--
                        	<a href="javascript:fncPopup('/cron/cron_maas.do?route=personel/personel_listesi','POPUP_CRON_MAAS',1100,850);" class="btn btn-outline-secondary btn-icon waves-effect waves-themed mr-1 text-white border-white" title="Maaşları Yansıt"> <i class="fal fa-lira-sign fs-xl"></i></a>
                        	-->
                            <a href="/personel/personel.do?route=<?=$_REQUEST['route']?>" class="btn btn-outline-secondary btn-icon waves-effect waves-themed mr-1 text-white border-white" title="Personel Cari Ekle"> <i class="far fa-plus"></i> </a>
							        ...
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td><?=$row->DEPARTMAN?></td>
									          	<td><?=$row->GOREV?></td>
									          	<td><?=$row->MAAS_DONEM?></td>
									          	<td><?=$row->IL?> / <?=$row->ILCE?></td>
									          	<td><?=$row->CEPTEL?></td>
									          	<td align="center"><?=$row->DURUM?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->TARIH)?></td>
									          	<td align="right"> 
									          		<a href="/finans/borc_dekontu.do?route=finans/borc_dekontu&cari_id=<?=$row->ID?>" class="btn btn-outline-primary btn-icon" title="Ödeme"> <i class="far fa-calculator"></i> </a>
									          		<a href="/personel/personel.do?route=<?=$_REQUEST['route']?>&id=<?=$row->ID?>&kod=<?=$row->KOD?>" class="btn btn-outline-primary btn-icon" title="Personel Düzenle"> <i class="far fa-edit"></i> </a> 
									          	</td>
									        </tr>
								        <?}?>
							        </tbody>

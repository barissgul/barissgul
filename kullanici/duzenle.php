<?
	...
	session_kontrol();
	$row				= $cSubData->getKullanici($_REQUEST);
	$rows_resim			= $cSubData->getKullaniciResimler($_REQUEST);
	$rows_adres			= $cSubData->getKullaniciAdresler($_REQUEST);
	kod_kontrol($row);
?>
<!DOCTYPE html>
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Kullanıcı Düzenle")?></li>
                        ...
                        <div class="panel-content">
							...
							<div class="row">
								<div class="col-xl-3 col-md-2 text-center mt-5">							        
									<?if(file_exists($cSabit->imgPathFile($row->RESIM_URL)) AND !is_null($row->RESIM_URL)){?>
										<a href="<?=$cSabit->imgPath($row->RESIM_URL)?>" class="magnific-popup-modal"> <img src="<?=$cSabit->imgPath($row->RESIM_URL)?>" class="img-responsive center-block" alt="<?=$row->RESIM_ADI?>" width="150px"> </a>
									<?}else{?>
										<img src="/img/150x150.png" title="<?=$cSabit->imgPath($row->RESIM_URL)?>" width="150px">
									<?}?>
									<br>
									...
									<br>
									<button type="button" class="btn btn-danger mt-5" onclick="fncSifreSmsGonder(this)" data-id="<?=$row->ID?>" data-kod="<?=$row->KOD?>"> <?=dil("Şifreyi SMS Gönder")?> </button><br>
									<button type="button" class="btn btn-danger mt-5" onclick="fncSifreMailGonder(this)" data-id="<?=$row->ID?>" data-kod="<?=$row->KOD?>"> <?=dil("Şifreyi Mail Gönder")?> </button>
								</div>
								<div class="col-xl-9 col-md-10">
									<ul class="nav nav-tabs justify-content-center" role="tablist">
		                               	<li class="nav-item"><a class="nav-link fs-md py-3 px-5 <?=($_REQUEST['tab']=='tab_site' OR !isset($_REQUEST['tab']))?'active':''?>" href="#tab_site" data-toggle="tab"> <?=dil("Kullanıcı Bilgileri")?> </a></li>
		                            ...
		                            <div class="tab-content p-3 shadow">
						              	...
						              	<form name="formKullaniciDuzenle" id="formKullaniciDuzenle">
											<input type="hidden" name="islem" id="islem" value="kullanici_kaydet">
							            	...
							            	<div class="row">
												...
												<div class="col-md-4 mb-2">
													<div class="form-group">
													  	...
													  	<input type="text" class="form-control" placeholder="" name="skype" id="skype" maxlength="45" value="<?=$row->SKYPE?>">
													</div>
												</div>
												<div class="w-100"></div>
												<div class="col-md-4 mb-2">        
									            ...
									            <div class="col-md-12 mb-2">
													<div class="form-group">
													  	...
													  	<textarea name="adres" id="adres" class="form-control" maxlength="255" rows="3"><?=$row->ADRES?></textarea>
													</div>
												</div>
												<div class="col-md-6 mb-2">
													<div class="form-group">
												...
												<div class="col-md-12 mb-2 text-center">
													<div class="form-group">
														<button type="button" class="btn btn-primary mt-3 w-25" onclick="fncKullaniciKaydet()"> <?=dil("Kaydet")?> </button>
													</div>
												</div>
											</div>
											</form>
					              		...
					              		<div class="tab-pane <?=($_REQUEST['tab']=='tab_adres')?'active':''?>" id="tab_adres">	
					              			<form name="formKullaniciAdres" id="formKullaniciAdres">
											<input type="hidden" name="islem" id="islem" value="kullanici_adres_ekle">
											<input type="hidden" name="id" id="id" value="<?=$row->ID?>">
											<input type="hidden" name="kod" id="kod" value="<?=$row->KOD?>">
							            	...
							            	<div class="row">
							            		<div class="col-md-12 mb-2">
									            	<div class="form-group">
													  	<label class="form-label"><?=dil("Adres Adı")?></label>
													  	<input type="text" class="form-control" placeholder="" name="adres_adi" id="adres_adi" maxlength="30" value="">
													</div>
												</div>
												<div class="col-md-6 mb-2">
													<div class="form-group">
													  	<label class="form-label"><?=dil("İl")?></label>
													  	<select name="adres_il_id" id="adres_il_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;" onchange="fncAdresIlceDoldur()">
													      	<?=$cCombo->Iller()->setSecilen()->setSeciniz()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
												<div class="col-md-6 mb-2">
													<div class="form-group">
													  	<label class="form-label"><?=dil("İlçe")?></label>
													  	<select name="adres_ilce_id" id="adres_ilce_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
													      	<?=$cCombo->Ilceler()->setSecilen()->setSeciniz()->getSelect("ID","AD")?>
													    </select>
													</div>
												</div>
									            <div class="col-md-12 mb-2">
													<div class="form-group">
													  	<label class="form-label"><?=dil("Adres")?></label>
													  	<textarea name="adres_adres" id="adres_adres" class="form-control" maxlength="255" rows="5"></textarea>
													</div>
												</div>
												<div class="col-md-12 mb-2 text-center">
													<div class="form-group">
														<button type="button" class="btn btn-primary mt-3 w-25" onclick="fncAdresEkle()"> <?=dil("Ekle")?> </button>
													</div>
												</div>
											</div>
											</form>
						              		...
						              		<div class="row">
							                	<div class="col-md-6">
											    	...
											    	<div class="panel-container">
								                        <div class="panel-content">
								                        	<div class="panel-tag py-2"><?=dil("Yüklü olan resimler")?></div>
											              	<table class="table table-condensed table-sm">
													            ...
													            <tbody>
													                ...
													                <tr>
													                  	<td><?=($key+1)?></td>
													                  	<td><a href="<?=$cSabit->imgPath($row_resim->URL)?>" class="magnific-popup-modal" data-gallery> <?=$row_resim->RESIM_ADI_ILK?> </a></td>
													                  	<td><?=FormatTarih::tarih($row_resim->TARIH)?></td>
													                  	<td class="p-0">
													                  		<a href="javascript:void(0)" class="btn btn-outline-danger btn-icon btn-sm" onclick="fncResimSil(this)" title="Sil" data-id="<?=$row_resim->ID?>"> <i class="far fa-trash"></i> </a>
													                  		&nbsp;&nbsp;
    ...
    <script>
		...
		function fncAdresEkle(){
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: $('#formKullaniciAdres').serialize(),
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){
		...
		function fncResimAktif(obj){
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: { "islem": "kullanici_resim_aktif", "id": $(obj).data("id"), "kod": $(obj).data("kod") },
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){
		...
		function fncSifreMailGonder(obj){
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: { "islem": "sifre_mail_gonder", "id": $(obj).data("id"), "kod": $(obj).data("kod") },
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){
		...
		function fncAdresIlceDoldur(){
			$("#adres_ilce_id").attr("disabled", "disabled");
			$.ajax({
				url: '/class/db_kayit.do?',
				type: "POST",
				data: { "islem" : "ilce_doldur", 'il_id' : $("#adres_il_id").val() },
				dataType: 'json',
				async: true,
				success: function(jd) {
					if(jd.HATA){
						...
						$('#adres_ilce_id').html(jd.HTML);
					}
					$("#adres_ilce_id").removeAttr("disabled");
				}
			});
		};

<?
	...
	$Table = $cTable->setSayfa("sayfaAlisFaturaRapor")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
	$rows = $Table['rows'];
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Alış Fatura Rapor")?></li>
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2><i class="fa fa-list mr-3"></i> <?=dil("Alış Fatura Rapor")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
                        <div class="panel-toolbar">
						  		...
						  		<table class="table table-sm table-condensed table-hover">
							  		<thead class="thead-themed fw-500">
								    	<tr>
								          	...
								          	<td align="right"><?=dil("Fatura Kdv")?></td>
								          	<td><?=dil("Açıklama")?></td>
								          	<td> </td>
								        </tr>
							        </thead>
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td> <?=$row->DOSYA_NO?></td>
									          	<td> <?=$row->FATURA_NO?></td>
									          	<td> <?=$row->FATURA_KES?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->FATURA_TARIH)?></td>
									          	<td align="right"> <?=FormatSayi::nokta2($row->FATURA_TUTAR)?></td>
									          	<td align="right"> <?=FormatSayi::nokta2($row->FATURA_KDV)?></td>
									          	<td> <?=$row->ACIKLAMA?></td>							          	
									          	<td align="center" class="p-0">
			                            			...
			                            			<a href="/finans/alis_fatura.do?route=finans/alis_faturalar&id=<?=$row->FATURA_ID?>&kod=<?=$row->FATURA_KOD?>" class="btn btn-outline-primary btn-icon" title="Düzenle"> <i class="far fa-edit"></i> </a>
									          	</td>
									        </tr>
								        <?}?>
							        </tbody>

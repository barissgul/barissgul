<?
	...
	if(is_null($_REQUEST['muhasebe_surec_id'])) $_REQUEST['muhasebe_surec_id'] = 1;
	$_REQUEST['filtre'] = 1;
	...
	$excel->sutunEkle("Ruhsat Sahibi","FATURA_TARIH","format1");
	$excel->sutunEkle("Fatura Tutar","FATURA_TUTAR","");
	$excel->sutunEkle("Ödeme Tutar","ODEME_TUTAR","");
	$excel->sutunEkle("Ödeme Talimat Tarihi","SIGORTA_ODEME_TALIMAT_TARIH","format1");
	$excel->sutunEkle("Kalan Tutar","KALAN_TUTAR","");
	$excelOut = $excel->excel();
	$Table = $cTable->setSayfa("sayfaSigortaOdemeBekleyenRapor")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
	$rows = $Table['rows'];
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Sigorta Ödeme Beklyenler")?></li>
                        ...
                        <div class="panel-content">
							<form name="form" id="form" class="app-forms" enctype="multipart/form-data" method="GET">
								...
								<div class="row">
									...
									<div class="col-xl-2 col-sm-4 mb-2">						            
									    <div class="form-group">
										  	<label class="form-label"><?=dil("Muhasebe Süreç")?></label>
										  	<select name="muhasebe_surec_id" id="muhasebe_surec_id" class="form-control select2 select2-hidden-accessible" style="width: 100%;">
										      	<?=$cCombo->MuhasebeSurecler()->setSecilen($_REQUEST['muhasebe_surec_id'])->setTumu()->getSelect("ID","AD")?>
										    </select>
										</div>
									</div>
								    <div class="col-xl-2 col-sm-4 mb-2">        
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2><i class="fa fa-list mr-3"></i> <?=dil("Sigorta Ödeme Beklyenler")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
                        <div class="panel-toolbar">
						  		...
						  		<table class="table table-sm table-condensed table-hover">
							  		<thead class="thead-themed fw-500">
								    	<tr>
								          	...
								          	<td align="right"><?=dil("Fatura Tutar")?></td>
								          	<td align="right"><?=dil("Ödeme Tutar")?></td>
								          	<td align="center" title="Ödeme Sayısı"><?=dil("ÖS")?></td>
								          	<td align="center"><?=dil("Ödeme Talimat Tarihi")?></td>
								          	<td align="right"><?=dil("Kalan Tutar")?></td>
								          	<td> </td>
								        </tr>
							        </thead>
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td align="right"> <?=$row->FATURA_TUTAR?> </td>
									          	<td align="right"> <?=$row->ODEME_TUTAR?></td>
									          	<td align="center"> <?=$row->ODEME_SAY?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->SIGORTA_ODEME_TALIMAT_TARIH)?></td>
									          	<td align="right"> <?=$row->KALAN_TUTAR?></td>
									          	<td align="center" class="p-0">
									          		<a href="/talep/talep.do?route=talep/talep_listesi&id=<?=$row->ID?>&kod=<?=$row->KOD?>&tab=tab_talep_notu" target="_blank" class="btn btn-outline-primary btn-icon waves-effect waves-themed" title="Özet"> <i class="fal fa-edit"></i></a>

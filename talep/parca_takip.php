<?
	...
	$excel->sutunEkle("Parça Sayısı","PARCA_SAYISI","");
	$excel->sutunEkle("Sipariş Tarih","SIPARIS_TARIH","format1");
	$excel->sutunEkle("Araç Geliş Tarihi","ARAC_GELIS_TARIH","format1");
	$excel->sutunEkle("Tah.Teslimi Tarihi ","TAHMINI_TESLIM_TARIH","format1");
	$excel->sutunEkle("Talep Tarih","TARIH","");
	$excel->sutunEkle("Son İşlem Tarih","GTARIH","");
	$excelOut = $excel->excel();
	$_REQUEST['filtre']		= 1;
	$Table = $cTable->setSayfa("sayfaParcaTakip")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
	$rows = $Table['rows'];
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Parça Takip")?></li>
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2> <i class="fas fa-cogs mr-3"></i> <?=dil("Parça Takip")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
						  		...
						  		<table class="table table-sm table-condensed table-hover datatable">
							  		<thead class="thead-themed fw-500">
								    	<tr>
								          	...
								          	<td><?=dil("Cari")?></td>
								          	<td><?=dil("Süreç")?></td>
								          	<td align="center"><?=dil("Parça Sayısı")?></td>
								          	<td align="center"><?=dil("Sipariş Tarihi")?></td>
								          	<td align="center"><?=dil("Araç Geliş Tarihi")?></td>
								          	<td align="center"><?=dil("Tah.Teslim Tarihi")?></td>								          	
								          	<td class="no-sort"> </td>
								        </tr>
							        </thead>
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td align="center"><?=$row->TALEP_ID?></td>
									          	<td><?=$row->PLAKA?></td>
									          	<td><?=$row->MARKA?> <?=$row->MODEL?></td>
									          	<td> <?=$row->CARI?></td>
									          	<td> <?=$row->SUREC?></td>
									          	<td align="center"><?=$row->PARCA_SAYISI?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->SIPARIS_TARIH)?></td>

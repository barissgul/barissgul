<?
	...
	$_REQUEST['filtre']		= 1;
	$Table = $cTable->setSayfa("sayfaOdemeTakip")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();	
	$rows = $Table['rows'];
...
<body class="mod-bg-1">
    ...
    <main id="js-page-content" role="main" class="page-content">
    	<ol class="breadcrumb page-breadcrumb breadcrumb-seperator-1">
            ...
            <li class="breadcrumb-item active"><?=dil("Ödeme Takip")?></li>
	    	...
	    	<div class="row">
	    		<div class="col-md-12">
    				...
    				<div class="panel-hdr bg-primary-300 text-white">
                        <h2> <i class="far fa-lira-sign mr-3"></i> <?=dil("Ödeme Takip")?> &nbsp;<span class="small">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
						  		...
						  		<table class="table table-sm table-condensed table-hover datatable">
							  		<thead class="thead-themed fw-500">
								    	<tr>
								          	...
								          	<td><?=dil("Süreç")?></td>
								          	<td><?=dil("Cari")?></td>
								          	<td align="right"><?=dil("Ödenecek Tutar")?></td>				          
								          	<td align="center"><?=dil("Teslim Edildi Tarih")?></td>
								          	<td align="center"><?=dil("Ödeme Tarihi")?></td>
							        ...
							        <tbody>
								        <?foreach($rows as $key=>$row) {?>
									        <tr>
									          	...
									          	<td> <?=$row->SUREC?></td>
									          	<td> <?=$row->CARI?> </td>
									          	<td align="right"> <?=FormatSayi::sayi($row->ODENECEK_TUTAR)?> <i class="fal fa-lira-sign"></i> </td>	
									          	<td align="center"><?=FormatTarih::tarih($row->TESLIM_EDILDI_TARIH)?></td>
									          	<td align="center"><?=FormatTarih::tarih($row->ODEME_YAPILDI_TARIH)?></td>
									          	...
									          	<td align="center" class="p-0">
			                            			...
			                            			<a href="<?=fncTeslimIbraPopupLink($row)?>" class="btn btn-outline-primary btn-icon waves-effect waves-themed" title="Teslim İbra"> <i class="fal fa-print"></i></a>
									          	</td>
									        </tr>
								        <?}?>
							        </tbody>

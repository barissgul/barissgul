<?
	...
	$excel->sutunEkle("İşlem No","FINANS_KALEMI","ID");
	...
	$excel->sutunEkle("İskonto","ISKONTO","");
	...
	$Table = $cTable->setSayfa("sayfaCariHareketDetaylar")->setExcel($excelOut)->setPost($_REQUEST)->Uygula()->getTable();
	$rows = $Table['rows'];
								...
								<div class="row">
									...
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
									      	...
									      	<input type="text" class="form-control" placeholder="" name="parca_kodu" id="parca_kodu" value="<?=$_REQUEST['parca_kodu']?>" maxlength="50">
									    </div>
									</div>
									<div class="col-md-2 mb-2">        
									    <div class="form-group">
									      	<label class="form-label"> <?=dil("Oem Kodu")?> </label>
									      	<input type="text" class="form-control" placeholder="" name="oem_kodu" id="oem_kodu" value="<?=$_REQUEST['oem_kodu']?>" maxlength="50">
									    </div>
									</div>									
		    	...
		    	<div class="col-md-12">
		    		<div class="panel">
	                    <div class="panel-hdr bg-primary-300 text-white">
	                        <h2> <i class="fal fa-list mr-3"></i> <?=dil("Talep Detaylar")?> &nbsp;<span style="font-size: 10px;">(<?=$Table['sayfaUstYazi']?>)</span> </h2>
									        ...
									        <tr>
									          	...
									          	<td align="center"> 
									          		<?if(in_array($_SESSION['yetki_id'], array(1,2,6))){?>
										          		<?if($row->HAREKET_ID == 1){?>
										          			<a href="/finans/satis_fatura.do?route=finans/satis_faturalar&id=<?=$row->CARI_HAREKET_ID?>&kod=<?=$row->CARI_HAREKET_KOD?>" class="btn <?=($row->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-xs"> <?=$row->ID?></a> 
										          		<?} else if($row->HAREKET_ID == 2){?>
										          			<a href="/finans/alis_fatura.do?route=finans/alis_faturalar&id=<?=$row->CARI_HAREKET_ID?>&kod=<?=$row->CARI_HAREKET_KOD?>" class="btn <?=($row->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-xs"> <?=$row->ID?></a> 
										          		<?} else if($row->HAREKET_ID == 3){?>
										          			<a href="/finans/tahsilat.do?route=finans/tahsilatlar&id=<?=$row->CARI_HAREKET_ID?>&kod=<?=$row->CARI_HAREKET_KOD?>" class="btn <?=($row->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-xs"> <?=$row->ID?></a> 
										          		<?} else if($row->HAREKET_ID == 4){?>
										          			<a href="/finans/tediye.do?route=finans/tediyeler&id=<?=$row->CARI_HAREKET_ID?>&kod=<?=$row->CARI_HAREKET_KOD?>" class="btn <?=($row->RESIM_VAR == 1)?'btn-primary':'btn-outline-primary'?> waves-effect waves-themed btn-xs"> <?=$row->ID?></a> 
										          		<?}?>
									          		<?} else {?>
									          			<?=$row->ID?>
									          		<?}?>
									          	...
									          	<td align="right"><?=FormatSayi::sayi($row->FIYAT,2)?></td>
									          	<td align="center"><?=$row->ADET?></td>
									          	<td align="center"><?=$row->ISKONTO?></td>
									          	...
									          	<td align="left" class="p-0">
									          		<?if(in_array($row->HAREKET_ID, array(1,4))){?>
									          			<a href="<?=fncFaturaPopupLink((object)array("ID"=>$row->CARI_HAREKET_ID,"KOD"=>$row->CARI_HAREKET_KOD))?>" class="btn btn-outline-primary btn-icon"> <i class="fal fa-eye"></i> </a>
									          		<?} else {?>
									          			<a href="<?=fncOdemePopupLink((object)array("ID"=>$row->CARI_HAREKET_ID,"KOD"=>$row->CARI_HAREKET_KOD))?>" class="btn btn-outline-primary btn-icon"> <i class="fal fa-eye"></i> </a>
									          		<?}?>
									          	</td>	
									        </tr>
								        <?}?>

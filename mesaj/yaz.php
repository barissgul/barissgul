				            ...
				            <div class="panel-container show">
				                <div class="panel-content">
									<form name="form" id="form" class="" enctype="multipart/form-data" method="GET">
			          					<input type="hidden" name="islem" value="mesaj_yaz">
										...
										<div class="row">
								            ...
								            <div class="col-md-12 mb-3">
								              	<div class="form-group">
								                	...
								                	<select name="kime_id" id="kime_id" class="form-control select2" style="width: 100%;" data-tags="true" data-placeholder="<?=dil("Kime")?>" data-allow-clear="true">
														...
														<?=$cCombo->Sorumlular()->setSecilen($_REQUEST['kime_id'])->setSeciniz()->getSelect("ID","AD")?>
														<?}else if($_SESSION['yetki_id'] < 4){?>
														<?=$cCombo->Kullanicilar()->setSecilen($_REQUEST['kime_id'])->setSeciniz()->getSelect("ID","AD")?>
														<?}?>
													</select>
								              	</div>
								            </div>
								            <div class="col-md-12 mb-3">
								              	<div class="form-group">
								                	...
								                	<input name="baslik" id="baslik" class="form-control" placeholder="<?=dil("Başlık")?>" value="<?=$_REQUEST['baslik']?>">
								              	</div>
								            </div>
								            <div class="col-md-12 mb-3">
								              	<div class="form-group">
								              		<label class="form-label"><?=dil("Konu")?></label>
								                    <textarea name="konu" id="konu" class="form-control" style="height: 300px"></textarea>
								              	</div>
								            </div>
											<div class="col-md-12 text-center">        
												<button type="button" class="btn btn-danger btn-ozel" onclick="fncMesajGonder()"><?=dil("Gönder")?></button>
											</div>
										</div>
									</form>
								</div>
			...
			$.ajax({
				...
				success: function(jd) {
					if(jd.HATA){
						...
						bootbox.alert(jd.ACIKLAMA, function() {
							location.href = "/mesaj/giden_kutusu.do?route=mesaj/giden_kutusu&sayfa=1";

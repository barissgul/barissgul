<?
	...
	$rows_mesaj 			= $cSubData->getMesajGelen();
?>
<!DOCTYPE html>
                ...
                <div class="position-absolute pos-top pos-bottom w-100">
                    <div class="d-flex h-100 flex-column">
                        ...
                        <div class="flex-1 pr-3">
                            <a href="/mesaj/gelen_kutusu.do?route=mesaj/gelen_kutusu&sayfa=1" class="px-3 px-sm-4 pr-lg-3 pl-lg-5 py-2 fs-md bg-primary-300 d-flex justify-content-between rounded-pill border-top-left-radius-0 border-bottom-left-radius-0 ">
                ...
                <div class="flex-grow-0">
                    <div class="d-flex align-items-center pl-2 pr-3 py-3 pl-sm-3 pr-sm-4 py-sm-4 px-lg-5 py-lg-4  border-faded border-top-0 border-left-0 border-right-0 flex-shrink-0">
                        ...
                        <h1 class="subheader-title ml-1 ml-lg-0">
                            <i class="fas fa-folder-open mr-2 hidden-lg-down"></i><?=dil("Gelen Kutusu")?>
                        </h1>
                                ...
                                <li class="<?=(($row_mesaj->OKUNDU==1)?'':'unread')?>">
                                    <div class="d-flex align-items-center px-3 px-sm-4 px-lg-5 py-1 py-lg-0 height-4 height-mobile-auto">
                                        <div class="custom-control custom-checkbox mr-3 order-1">
                                            <input type="checkbox" class="custom-control-input mesaj_ckeck" id="mesaj_ckeck<?=$row_mesaj->ID?>" name="mesaj_ckeck<?=$row_mesaj->ID?>" value="<?=$row_mesaj->ID?>">
                                            <label class="custom-control-label" for="mesaj_ckeck<?=$row_mesaj->ID?>"></label>
                                        </div>
                                        ...
                                        <div class="d-flex flex-row flex-wrap flex-1 align-items-stretch align-self-stretch order-2 order-lg-3">
                                            <div class="row w-100">
                                                <a href="/mesaj/oku.do?route=mesaj/oku&id=<?=$row_mesaj->ID?>" class="name d-flex width-sm align-items-center pt-1 pb-0 py-lg-1 col-12 col-lg-auto"><?=$row_mesaj->KIMDEN?></a>
                                        ...
                                        <!--
                                        <div class="fs-sm text-muted ml-auto hide-on-hover-parent order-4 position-on-mobile-absolute pos-top pos-right mt-2 mr-3 mr-sm-4 mt-lg-0 mr-lg-0"><?=FormatTarih::tarih($row_mesaj->OKUNDU_TARIH)?></div>
                                        -->
                                    </div>
                                </li>
                                <?}?>
			...
			$.ajax({
				...
				success: function(jd) {
					if(jd.HATA){
						...
						bootbox.alert(jd.ACIKLAMA, function() {window.location.reload();});
